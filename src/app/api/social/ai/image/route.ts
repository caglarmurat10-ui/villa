...
export async function GET(request: Request) {
  ...
  } catch (error) { return Response.json({ error: publicAiError(error, "AI medya durumu alınamadı.") }, { status: 400 }); }
}
export async function POST(request: Request) {
  ...
  } catch (error) { return Response.json({ error: publicAiError(error, "AI görseli oluşturulamadı.") }, { status: 400 }); }
}
