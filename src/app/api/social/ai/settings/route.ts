...
export async function GET(request: Request) {
  try {
    ...
    const items = await Promise.all(villas.map(async (villa) => ({ settings: await getAiSettings(db, villa),
      profile: await getVillaAiProfile(db, villa),
      systemFlags: { image: String(env.AI_IMAGE_ENABLED) === "true", video: String(env.AI_VIDEO_ENABLED) === "true" } })));
    return Response.json({ items, usage: await aiUsageSummary(db) });
