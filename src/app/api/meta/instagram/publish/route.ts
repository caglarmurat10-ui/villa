...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return Response.json({ error: parsed.error.issues[0]?.message ?? "Geçersiz paylaşım." }, { status: 400 });
  }
  ...
  if (post.approvalStatus !== "Onaylandı") {
    return Response.json({ error: "Instagram yayını için önce insan onayı verilmelidir." }, { status: 409 });
  }
  if (!post.mediaUrl) {
  ...
  try {
    ...
    if (!account) {
      return Response.json({ error: `Villa ${post.villa} Instagram hesabı bağlı değil.` }, { status: 409 });
    }
