...
function safeError(error: unknown) {
  const message = error instanceof Error && error.message ? error.message : "Facebook marka görselleri uygulanamadı.";
  return message
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const applied = Number(result.profile.applied) + Number(result.cover.applied);
