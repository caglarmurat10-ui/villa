...
export async function metaConfig() {
  ...
  if (!appId) {
    throw new Error("Eksik ortam değişkeni: META_APP_ID");
  }

  if (!appSecret) {
    throw new Error("Eksik ortam değişkeni: META_APP_SECRET");
  }

  if (!baseUrl) {
    throw new Error("Eksik ortam değişkeni: APP_BASE_URL");
  }
...
export async function verifyInstagramState(state: string) {
  ...
  if (expected !== state || !/^[a-f0-9]{64}$/.test(signature)) {
    return null;
  }
...
export async function exchangeInstagramCode(code: string) {
  ...
  if (!response.ok || !data.access_token) {
    throw new Error(
      data.error_message ?? "Instagram erişim anahtarı alınamadı.",
    );
  }
...
export async function getInstagramProfile(accessToken: string) {
  ...
  if (!response.ok || !data.id) {
    throw new Error(
      data.error?.message ?? "Instagram profili alınamadı.",
    );
  }
...
export async function publishInstagramImage(
  ...
  if (!publish.ok || !published.id) {
    throw new Error(
      published.error?.message ?? "Instagram yayını başarısız.",
    );
  }
