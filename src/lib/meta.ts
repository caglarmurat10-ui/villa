...
export async function metaConfig() {
  ...
  if (!appId || !appSecret || !baseUrl) throw new Error("Meta bağlantı ayarları tamamlanmamış.");
...
export async function exchangeInstagramCode(code: string) {
  ...
  const response = await fetch(INSTAGRAM_TOKEN, { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body });
  const data = await response.json() as { access_token?: string; user_id?: number | string; error_message?: string };
...
export async function publishInstagramImage(accountId: string, accessToken: string, imageUrl: string, caption: string) {
