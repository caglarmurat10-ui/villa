...
export type ProspectStatus = "DISCOVERED" | "WATCHLIST" | "RECOMMENDED" | "DISMISSED" | "BLOCKED";
...
export type AgentRunStatus = "OK" | "ERROR" | "PENDING_PERMISSION";
...
type ProspectRow = {
  ...
  display_name: string | null; category: ProspectCategory; bio_summary: string | null;
...
function mapProspect(row: ProspectRow): SocialProspect {
  return {
    ...
    displayName: row.display_name, category: row.category, bioSummary: row.bio_summary,
...
export async function listProspects(villa?: Villa | null, limit = 50): Promise<SocialProspect[]> {
  const db = await database();
  await ensureTables(db);
  const safeLimit = Math.max(1, Math.min(200, limit));
  const sql = villa
    ? `SELECT * FROM social_prospects WHERE villa = ? ORDER BY final_growth_score DESC, discovered_at DESC LIMIT ?`
    : `SELECT * FROM social_prospects ORDER BY final_growth_score DESC, discovered_at DESC LIMIT ?`;
  const statement = villa ? db.prepare(sql).bind(villa, safeLimit) : db.prepare(sql).bind(safeLimit);
  const result = await statement.all<ProspectRow>();
...
export async function upsertProspect(input: NewProspectInput): Promise<SocialProspect> {
  ...
  await db.prepare(`INSERT INTO social_prospects
    (id, villa, platform, username, account_id, display_name, category, bio_summary, followers_count, media_count,
     location_hint, relevance_score, engagement_score, location_score, audience_fit_score, spam_risk_score,
     final_growth_score, discovered_at, last_checked_at, status, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(platform, username) DO UPDATE SET
      villa=excluded.villa, account_id=excluded.account_id, display_name=excluded.display_name,
      ...
      final_growth_score=excluded.final_growth_score, last_checked_at=excluded.last_checked_at, updated_at=excluded.updated_at`)
    .bind(
      id, input.villa, input.platform, input.username, input.accountId, input.displayName, input.category,
      ...
      input.discoveredAt, input.lastCheckedAt, input.status ?? "DISCOVERED", now, now,
    ).run();
