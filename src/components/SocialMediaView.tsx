...
// custom-worker.mjs'deki MAX_ATTEMPTS ile aynı deger olmali - duePosts() attempt_count >= 3 olan
// satirlari cron'a bir daha sunmuyor, bu yalnizca ayni esigi admin'de gorunur kilan bir rozet.
const SOCIAL_MAX_ATTEMPTS = 3;
...
export default function SocialMediaView({ initialPosts, availabilityGaps }: { initialPosts: SocialPost[]; availabilityGaps: AvailabilityGap[] }) {
  ...
  const availableMedia = useMemo(() => socialDriveMedia.filter((asset) => {
    if (asset.villa !== villa) return false;
    if (contentType === "Reels") return asset.mediaKind === "video";
    if (platform === "Facebook" && contentType === "Gönderi") return asset.mediaKind === "image";
    return true;
  }), [villa, platform, contentType]);
  ...
  useEffect(() => {
    function handleTemplate(event: Event) {
      ...
      setSelectedMedia(template.mediaUrl ? [template.mediaUrl] : []);
      setManualMediaUrl("");
      setNotice(template.mediaResolved
        ? `${template.id} yüklendi. ${template.mediaFile} Drive'dan doğrulandı ve doğru villa medyası otomatik bağlandı.`
        : `${template.id} yüklendi ancak ${template.mediaFile} için doğrulanmış Drive medyası bulunamadı.`);
    }
  ...
  function changeVilla(next: Villa) {
    setVilla(next);
    setSelectedMedia([]);
    setManualMediaUrl("");
  }

  function changeContentType(next: SocialContentType) {
    setContentType(next);
    setSelectedMedia((current) => (next === "Reels" || next === "Hikâye" ? current.slice(0, 1) : current.slice(0, 10)));
  }

  function toggleMedia(asset: DriveMediaAsset) {
    const url = asset.proxyPath;
    setManualMediaUrl("");
    setSelectedMedia((current) => {
      if (contentType === "Reels" || contentType === "Hikâye") return current[0] === url ? [] : [url];
      if (current.includes(url)) return current.filter((item) => item !== url);
      if (current.length >= 10) { setNotice("Carousel en fazla 10 medya içerebilir."); return current; }
      return [...current, url];
    });
  }

  function useGap(gap: AvailabilityGap) {
    changeVilla(gap.villa);
  ...
  async function addPost(e: FormEvent) {
    ...
    try {
      const rawMedia = selectedMedia.length ? selectedMedia : (manualMediaUrl ? [manualMediaUrl] : []);
      const mediaUrls = rawMedia.map(absoluteMediaUrl);
      const r=await fetch("/api/social-posts",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({villa,platform,contentType,scheduledDate,caption,mediaUrl:mediaUrls[0]??"",mediaUrls})});
      const d=await r.json().catch(()=>({}));
      if(r.ok){setPosts(p=>sort([d.post,...p]));setCaption("");setSelectedMedia([]);setManualMediaUrl("");setNotice(mediaUrls.length>1?`Carousel ${mediaUrls.length} doğrulanmış medyayla plana eklendi. Yayınlamadan önce insan onayı gerekli.`:"Paylaşım gerçek medyayla plana eklendi. Yayınlamadan önce insan onayı gerekli.");}
  ...
  async function approval(post: SocialPost) {
    ...
    try {
      ...
      const d=await r.json().catch(()=>({}));
      if(r.ok){setPosts(p=>sort(p.map(x=>x.id===post.id?d.post:x)));setNotice(next==="Onaylandı"?"Paylaşım insan onayından geçti. Yayına ve otomatik cron'a hazır.":"İnsan onayı kaldırıldı.");}
  ...
  async function publishMeta(post: SocialPost, target: "Instagram" | "Facebook") {
    if (post.approvalStatus !== "Onaylandı") { setNotice(`${target} yayını için önce insan onayı verin.`); return; }
    if (target === "Instagram" && !post.mediaUrl) { setNotice("Instagram yayını için doğrulanmış medya gerekli."); return; }
    if (target === "Facebook" && post.contentType === "Hikâye") { setNotice("Facebook Story otomatik yayını güvenli Meta endpointi doğrulanana kadar manuel kalacak."); return; }
    if (target === "Facebook" && post.contentType === "Reels" && !post.mediaUrl) { setNotice("Facebook Reels için doğrulanmış video gerekli."); return; }
    setPublishing(post.id); setNotice("");
    try {
      const endpoint = target === "Instagram" ? "/api/meta/instagram/publish" : "/api/meta/facebook/publish";
      const r=await fetch(endpoint,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({postId:post.id})});
      const d=await r.json().catch(()=>({}));
      if(!r.ok){setNotice(d.error??`${target} yayını başarısız.`);return;}
      if(d.post) setPosts(p=>sort(p.map(x=>x.id===post.id?d.post:x)));
      const format = d.mediaCount && d.mediaCount > 1 ? ` · Carousel ${d.mediaCount} medya` : ` · ${post.contentType}`;
      setNotice(`${target} yayını gönderildi${d.username?` (${target === "Instagram" ? "@" : ""}${d.username})`:""}${format}.`);
    } catch { setNotice(`${target} bağlantısı kurulamadı.`); } finally { setPublishing(null); }
  }
  ...
  const selectedPreview = selectedMedia.map((url) => socialDriveMedia.find((asset) => asset.proxyPath === url)).filter(Boolean) as DriveMediaAsset[];
  ...
  return <main className="social-page"><div className="social-top"><a href="/">← Ana panele dön</a><span>Villa Yönetim</span></div><section className="social-panel">
    <div className="social-hero"><div><span className="eyebrow">SOSYAL MEDYA MERKEZİ</span><h1>Boş tarihleri içeriğe dönüştür</h1><p>Villa Safira ve Villa Destan için rezervasyon takviminden kampanya üretin, gerçek medyayı seçin, insan onayından geçirin ve Meta'ya yayınlayın.</p></div><div className="social-overview"><article><span>Planlanan</span><strong>{planned}</strong></article><article><span>Onay bekleyen</span><strong>{approvalPending}</strong></article><article><span>Yayınlanan</span><strong>{published}</strong></article></div></div>
    ...
    <div className="social-layout"><form className="social-compose" onSubmit={addPost}><div className="social-section-title"><h2>Yeni paylaşım</h2><small>{caption.length}/2200</small></div>
      <div className="two"><label>Villa<select value={villa} onChange={e=>changeVilla(e.target.value as Villa)}><option value="Safira">Villa Safira</option><option value="Destan">Villa Destan</option></select></label><label>Platform<select value={platform} onChange={e=>{const n=e.target.value as SocialPlatform;setPlatform(n);setSelectedMedia([]);if(!types[n].includes(contentType))changeContentType(types[n][0]);}}>{platforms.map(x=><option key={x}>{x}</option>)}</select></label></div>
      <div className="two"><label>Paylaşım türü<select value={contentType} onChange={e=>changeContentType(e.target.value as SocialContentType)}>{types[platform].map(x=><option key={x}>{x}</option>)}</select></label><label>Planlanan tarih<input type="date" min={today()} value={scheduledDate} onChange={e=>setScheduledDate(e.target.value)} required/></label></div>
      ...
      {(platform==="Instagram"||platform==="Facebook")?<div className="media-picker"><div className="social-section-title"><h3>Doğrulanmış Villa medyası</h3><small>{selectedMedia.length}{contentType==="Gönderi"?"/10":"/1"}</small></div><p className="media-picker-help">{contentType==="Reels"?"Yalnız gerçek video seçilebilir.":contentType==="Hikâye"?"Tek görsel veya video seçin.":"Tek görsel Gönderi; 2–10 seçim otomatik Carousel olur."}</p><div className="media-picker-grid">{availableMedia.length===0?<div className="empty">Bu villa ve format için doğrulanmış medya henüz registry'de görünmüyor.</div>:availableMedia.map(asset=>{const chosen=selectedMedia.includes(asset.proxyPath);return <button type="button" className={`media-pick ${chosen?"selected":""}`} key={asset.fileId} onClick={()=>toggleMedia(asset)}>{asset.mediaKind==="video"?<div className="media-video-placeholder">▶ VIDEO</div>:<img src={asset.previewUrl} alt={asset.fileName}/>}<span>{asset.fileName}</span><b>{chosen?"✓ Seçildi":asset.mediaKind==="video"?"Reels/Story":"Seç"}</b></button>})}</div></div>:<label>Medya bağlantısı<input type="url" placeholder="İsteğe bağlı medya bağlantısı" value={manualMediaUrl} onChange={e=>setManualMediaUrl(e.target.value)} /></label>}

      {selectedPreview.length>0?<div className="selected-media-strip">{selectedPreview.map((asset,index)=><div key={asset.fileId} className="selected-media-item"><span>{index+1}</span>{asset.mediaKind==="video"?<div className="media-video-placeholder">▶</div>:<img src={asset.previewUrl} alt=""/>}<small>{asset.fileName}</small></div>)}</div>:null}
      <div className="social-preview"><div className="social-preview-media">{selectedPreview[0]?(selectedPreview[0].mediaKind==="video"?<span>▶ {selectedPreview[0].fileName}<br/>Gerçek video · yayın sırasında Meta tarafından işlenecek</span>:<img src={selectedPreview[0].previewUrl} alt={`${villaName(villa)} paylaşım önizlemesi`} />):<span>Doğrulanmış Drive medyası seçildiğinde burada görünür.</span>}</div><div className="social-preview-copy"><strong>{villaName(villa)}</strong><small>{platform} · {selectedMedia.length>1?`Carousel (${selectedMedia.length})`:contentType}</small><p>{caption||"Paylaşım metni önizlemesi"}</p></div></div>
      ...
      <div className="social-post-list">{visible.length===0?<div className="empty">Henüz paylaşım yok.</div>:visible.map(post=><article className={`social-post ${post.status==="Yayınlandı"?"published":""}`} key={post.id}>{post.mediaUrl?<img className="social-post-image" src={post.mediaUrl} alt="" />:null}<div className="social-post-top"><div className="social-badges"><span className={`platform-badge ${tone[post.platform]}`}>{post.platform}</span><span>{post.contentType}</span><span>{villaName(post.villa)}</span></div><div className="social-state-badges"><span className={`approval-status ${post.approvalStatus==="Onaylandı"?"approved":"pending"}`}>{post.approvalStatus==="Onaylandı"?"✓ Onaylandı":"İnsan onayı"}</span><span className={`social-status ${post.status==="Yayınlandı"?"done":"planned"}`}>{post.status}</span></div></div>{post.status==="Planlandı"&&(post.publishAttemptCount??0)>=SOCIAL_MAX_ATTEMPTS?<p className="social-retry-exhausted" title={post.lastPublishError??undefined}>⚠ Otomatik yayın {SOCIAL_MAX_ATTEMPTS} denemede başarısız oldu, tekrar denenmiyor — manuel kontrol gerekli{post.lastPublishError?`: ${post.lastPublishError}`:""}</p>:null}<strong className="social-date">{trDate(post.scheduledDate)}</strong><p className="social-caption">{post.caption}</p><div className="social-actions"><button onClick={()=>copy(post)}>Metni kopyala</button>{post.status==="Planlandı"?<button className={`approval-action ${post.approvalStatus==="Onaylandı"?"approved":""}`} disabled={approving===post.id||publishing===post.id} onClick={()=>approval(post)}>{approving===post.id?"İşleniyor…":post.approvalStatus==="Onaylandı"?"Onayı kaldır":"Onay ver ✓"}</button>:null}{post.platform==="Instagram"&&post.status==="Planlandı"?<button className="publish-action" disabled={publishing===post.id||post.approvalStatus!=="Onaylandı"||!post.mediaUrl} onClick={()=>publishMeta(post,"Instagram")}>{publishing===post.id?"Yayınlanıyor…":post.approvalStatus!=="Onaylandı"?"Önce onay gerekli":`Instagram ${post.contentType} yayınla`}</button>:post.platform==="Facebook"&&post.status==="Planlandı"&&post.contentType!=="Hikâye"?<button className="publish-action facebook-publish" disabled={publishing===post.id||post.approvalStatus!=="Onaylandı"||(post.contentType==="Reels"&&!post.mediaUrl)} onClick={()=>publishMeta(post,"Facebook")}>{publishing===post.id?"Yayınlanıyor…":post.approvalStatus!=="Onaylandı"?"Önce onay gerekli":`Facebook ${post.contentType} yayınla`}</button>:<button className="status-action" disabled={post.status==="Planlandı"&&post.approvalStatus!=="Onaylandı"} onClick={()=>status(post)}>{post.status==="Planlandı"?"Yayınlandı ✓":"Plana geri al"}</button>}<button className="delete" onClick={()=>remove(post)}>Sil</button></div></article>)}</div></div>
    </div></section></main>;
}
