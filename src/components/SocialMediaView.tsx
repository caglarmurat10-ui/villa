...
export default function SocialMediaView({ initialPosts, availabilityGaps }: { initialPosts: SocialPost[]; availabilityGaps: AvailabilityGap[] }) {
  ...
  useEffect(() => {
    function handleTemplate(event: Event) {
      ...
      if (!template) return;
      const resolvedMediaUrl = template.mediaUrl ? new URL(template.mediaUrl, window.location.origin).toString() : "";
      ...
      setMediaPreviewUrl(template.previewUrl || resolvedMediaUrl);
  ...
  async function addPost(e: FormEvent) {
    ...
    try {
      ...
      if(r.ok){setPosts(p=>sort([d.post,...p]));setCaption("");setMediaUrl("");setMediaPreviewUrl("");setNotice("Paylaşım gerçek medyayla plana eklendi. Yayınlamadan önce insan onayı gerekli.");}
  ...
  async function publishMeta(post: SocialPost, target: "Instagram" | "Facebook") {
    ...
    if (target === "Instagram" && post.contentType !== "Gönderi") { setNotice("Instagram doğrudan yayını şu anda gerçek görsel Gönderi formatı için aktif. Reels/Hikâye video akışında hazırlanacak."); return; }
  ...
  return <main className="social-page"><div className="social-top"><a href="/">← Ana panele dön</a><span>Villa Yönetim</span></div><section className="social-panel">
    ...
    <div className="social-layout"><form className="social-compose" onSubmit={addPost}><div className="social-section-title"><h2>Yeni paylaşım</h2><small>{caption.length}/2200</small></div>
      ...
      <label>Görsel bağlantısı<input type="url" placeholder="Drive medyası seçilince otomatik dolar" value={mediaUrl} onChange={e=>{setMediaUrl(e.target.value);setMediaPreviewUrl(e.target.value);}} /></label>
      <div className="social-preview"><div className="social-preview-media">{mediaPreviewUrl?<img src={mediaPreviewUrl} alt={`${villaName(villa)} paylaşım önizlemesi`} />:<span>İçerik kütüphanesinden seçim yapınca doğrulanmış Drive medyası burada görünür.</span>}</div><div className="social-preview-copy"><strong>{villaName(villa)}</strong><small>{platform} · {contentType}</small><p>{caption||"Paylaşım metni önizlemesi"}</p></div></div>
